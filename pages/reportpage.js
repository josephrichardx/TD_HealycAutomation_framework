...
class SalesReportPage {
  ...
  async resetInvoiceStatus(frame) {
    ...
    await StepHelper.step(
      this.page,
      `Unselect Invoice Status - ${currentStatus}`,
      async () => {
        const selectedStatus = frame.locator(
          `//span[normalize-space()='${currentStatus}']`
        );

        await selectedStatus.waitFor({
          state: 'visible',
          timeout: timeout.elementTimeout
        });
  ...
  async getHoverDetails(frame) {
    ...
    for (let i = 1; i < count; i++) {
      ...
      // Wait for hover popup instead of fixed wait
      await expect(
